...
import pisak
from pisak import unit, res, properties, scanning, configurator, \
     utils, media, style, layout, svg, logger
...
class Button(Mx.Button, properties.PropertyAdapter, scanning.StylableScannable,
    ...
    def _insert_padding(self):
        ...
        padding = Clutter.Actor()
        padding.set_width(padding_width)
        if self.custom_padding == "left":
            where = 0
        elif self.custom_padding == "right":
            where = -1
        original_box.add_actor(padding, where)
