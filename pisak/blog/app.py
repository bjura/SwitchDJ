...
import pisak.viewer.model   #@UnusedImport
...
def prepare_main_view(stage, script, data):
    ...
    handlers.button_to_view(stage, script, "button_others", "blog/followed_blogs")
    handlers.button_to_view(stage, script, "button_my_blog", "blog/all_posts")
...
def prepare_about_me_view(stage, script, data):
    handlers.button_to_view(stage, script, "button_edit_desc",
                            "blog/speller_about_me")
    handlers.button_to_view(stage, script, "button_edit_photo",
                            "blog/viewer_about_me_library")
    handlers.button_to_view(stage, script, "button_exit")
    handlers.button_to_view(stage, script, "button_back", "blog/main")
...
def prepare_all_posts_view(stage, script, data):
    wordpress.blog.pending_post = None
    handlers.button_to_view(stage, script, "button_exit")
    handlers.button_to_view(stage, script, "button_about", "blog/about_me")
    handlers.button_to_view(stage, script, "button_back", "blog/main")
    handlers.button_to_view(
        stage, script, "button_new_post", "blog/speller_post")
    posts_data = script.get_object("posts_data")
    posts_data.item_handler = lambda tile, post: stage.load_view(
...
def prepare_single_post_view(stage, script, data):
    ...
    handlers.button_to_view(stage, script, "button_exit")
    handlers.button_to_view(stage, script, "button_back", "blog/all_posts")
    handlers.button_to_view(stage, script, "button_about", "blog/about_me")
    handlers.button_to_view(stage, script, "button_delete_post",
                            "blog/all_posts")
    handlers.button_to_view(stage, script, "button_post_edit",
                            "blog/speller_post")
...
def prepare_followed_blog_single_post_view(stage, script, data):
    post = data["post"]
    posts = data["posts"]
    content = script.get_object("post_text")
    ...
    handlers.button_to_view(stage, script, "button_exit")
    handlers.button_to_view(
        stage, script, "button_back", "blog/followed_blog_all_posts", data)
    handlers.button_to_view(stage, script, "button_about", "blog/about_me")
    handlers.button_to_view(
        stage, script, "button_comment", "blog/speller_comment", data)
    handlers.connect_button(script, "button_next_post", load_new_post, 1)
    handlers.connect_button(script, "button_previous_post", load_new_post,
                            -1)


def prepare_followed_blog_all_posts_view(stage, script, data):
    handlers.button_to_view(stage, script, "button_exit")
    handlers.button_to_view(stage, script, "button_about", "blog/about_me")
    handlers.button_to_view(stage, script, "button_back", "blog/followed_blogs")
...
def prepare_followed_blogs_view(stage, script, data):
    handlers.button_to_view(stage, script, "button_exit")
    handlers.button_to_view(stage, script, "button_about", "blog/about_me")
    handlers.button_to_view(stage, script, "button_back", "blog/main")
    blogs_data = script.get_object("blogs_data")
    blogs_data.item_handler = lambda tile, blog_url, blog_name: \
        stage.load_view("blog/followed_blog_all_posts",
                        {"blog_url": blog_url, "blog_name": blog_name})


def prepare_speller_post_view(stage, script, data):
    ...
    handlers.button_to_view(
        stage, script, "button_proceed", "blog/speller_title")
    handlers.button_to_view(stage, script, "button_exit")


def prepare_speller_title_view(stage, script, data):
    text_field = script.get_object("text_box")
    post = wordpress.blog.pending_post
    if post.title and post.title != "Untitled":
        text_field.type_text(post.title)
    handlers.button_to_view(
        stage, script, "button_proceed", "blog/viewer_library")
    handlers.button_to_view(stage, script, "button_exit")


def prepare_viewer_library_view(stage, script, data):
    ...
    handlers.button_to_view(
        stage, script, "button_publish_blog", "blog/all_posts")


def prepare_viewer_album_view(stage, script, data):
    library = pisak.viewer.model.get_library()
    def attach_photo(tile):
        wordpress.blog.post_images.append(tile.preview_path)
    handlers.button_to_view(
        stage, script, "button_library", "blog/viewer_library")
    handlers.button_to_view(
        stage, script, "button_publish_blog", "blog/all_posts")
    handlers.button_to_view(stage, script, "button_start")
...
def prepare_speller_about_me_view(stage, script, data):
    ...
    handlers.button_to_view(stage, script, "button_proceed", "blog/about_me")
    handlers.button_to_view(stage, script, "button_exit")


def prepare_viewer_about_me_library_view(stage, script, data):
    library_data = script.get_object("library_data")
    library_data.item_handler = lambda tile, album: stage.load_view(
...
def prepare_viewer_about_me_album_view(stage, script, data):
    library = pisak.viewer.model.get_library()
    def pick_photo(tile):
        wordpress.blog.update_about_me_photo(tile.preview_path)
        stage.load_view("blog/about_me")
    handlers.button_to_view(stage, script, "button_library",
                            "blog/viewer_about_me_library")
    handlers.button_to_view(stage, script, "button_start")
    album_id = data["album_id"]
    header = script.get_object("header")
...
def prepare_speller_comment_view(stage, script, data):
    def publish_comment():
    ...
    handlers.button_to_view(
         stage, script, "button_proceed", "blog/followed_blog_single_post", data)
